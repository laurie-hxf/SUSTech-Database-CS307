...
//        }
       ...
       // 真正删除数据文件和目录
        File tableDir = new File(ROOT_DIR, tableName);
        if (tableDir.exists()) {
            deleteDirectoryRecursively(tableDir);
        }

    }

    private void deleteDirectoryRecursively(File file) throws DBException {
        if (file.isDirectory()) {
        ...
        if (!file.delete()) {
            throw new DBException(ExceptionTypes.BadIOError("Cannot delete " + file.getAbsolutePath()));
        }
    }
