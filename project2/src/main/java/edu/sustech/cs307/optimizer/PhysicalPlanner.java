...
public class PhysicalPlanner {
    ...
    public static PhysicalOperator generateOperator(DBManager dbManager, LogicalOperator logicalOp) throws DBException {
        if (logicalOp instanceof LogicalTableScanOperator tableScanOperator) {
            ...
            return handleAggregateOperator(dbManager, aggregateOperator);
        } else if (logicalOp instanceof LogicalGroupByOperator groupbyOperator) {
            return handleGroupBy(dbManager, groupbyOperator);
        } else if (logicalOp instanceof LogicAntiJoinOperator antiJoinOperator) {
            return handleAntiJoin(dbManager, antiJoinOperator);
        }
        else if (logicalOp instanceof LogicalDeleteOperator del) {
            PhysicalOperator scanner = generateOperator(dbManager, new
                    LogicalTableScanOperator(del.getTableName(), dbManager));
            return new DeleteOperator(scanner, del.getTableName(), del.getWhereExpr());
        }
    ...
    private static PhysicalOperator handleTableScan(DBManager dbManager, LogicalTableScanOperator logicalTableScanOp) throws DBException {
        ...
        if (index&&tableMeta.getIndexes() != null && !tableMeta.getIndexes().isEmpty()) {
            String indexColumn = tableMeta.getIndexes().keySet().iterator().next();
            return new InMemoryIndexScanOperator(tableName,indexColumn,dbManager);
        } else {
            return new SeqScanOperator(tableName, dbManager);
        }
    }

    private static PhysicalOperator handleAntiJoin(DBManager dbManager, LogicAntiJoinOperator antiJoinOp)
            throws DBException {
        PhysicalOperator inputOp = generateOperator(dbManager, antiJoinOp.getChild());
        return new AntiJoinOperator(inputOp, antiJoinOp.getLeftExpression(), antiJoinOp.getRightExpression(), antiJoinOp.isin(),dbManager);
    }
    ...
    private static PhysicalOperator handleFilter(DBManager dbManager, LogicalFilterOperator logicalFilterOp)
        ...
        if(dbManager.isEn_index()){
            return new FilterIndexOperator(inputOp, logicalFilterOp.getWhereExpr(),dbManager);
        }else{
    ...
    private static PhysicalOperator handleGroupBy(DBManager dbManager, LogicalGroupByOperator logicalGroupByOp)
        ...
        return new GroupByOperator(inputOp, logicalGroupByOp.getGroupByExpr());
    }
...
//        return finalOp;
    ...
    private static PhysicalOperator handleAggregateOperator(DBManager dbManager, LogicalAggregateOperator logicalAggregateOperator) throws DBException {
        PhysicalOperator inputOp = generateOperator(dbManager, logicalAggregateOperator.getChild());
        return new AggregateOperator(inputOp, logicalAggregateOperator.getOutputSchema());
    }
