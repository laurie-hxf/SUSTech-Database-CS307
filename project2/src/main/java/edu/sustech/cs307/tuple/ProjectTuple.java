...
public class ProjectTuple extends Tuple {
    ...
    public Value[] getValues() throws DBException {
        ...
        for (var tabCol : this.schema) {
            if(inputTuple instanceof AggregateTuple temp){
                value = getValue(tabCol);
                if(value == null){
                    Tuple tuple = temp.getTuple();
                    value = tuple.getValue(tabCol);
                }
            }
            else {
                value = getValue(tabCol);
...
//        values.addAll(Arrays.asList(inputTuple.getValues()));
